...
func buildSA(data []byte) []int {
	return qsufsort(data)

}

func buildISA(sa []int) []int {
	isa := make([]int, len(sa))
	for i, val := range sa {
		isa[val] = i
	}
...
func buildLCP(T []byte, L []int, R []int) []int {
	n := len(T)
	lcp := make([]int, n)
	l := 0
	for i := 0; i < n-1; i++ {
		r := R[i]
		prevSuffix := L[r-1]
		for (i+l) < len(T) && (prevSuffix+l) < len(T) && T[i+l] == T[prevSuffix+l] {
			l++
		}
		lcp[r] = l
		l = max(l-1, 0)
